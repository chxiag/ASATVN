...
def generate_regular_PE(length: int, d_model: int, period: Optional[int] = 24) -> torch.Tensor:
    ...
    Generate positional encoding with a given period.
     :param length: a higher dimensional space L of the length of input data.
     :param d_model: Dimension of the model vector.
    ...
    return PE
class Truncated_Cauchy_self_attention_block(nn.Module):
    """
      Contains Truncated Cauchy self-attention block described in paper.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_discriminator:
            num_dims = len(output.shape)  # 3
            reduction_dims = tuple(range(1, num_dims))  # （1,2）
