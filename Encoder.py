...
def get_dist_mask_tile(sentence_len):
    """
        Calculate Truncated Cauchy matrix
...
class Truncated_Cauchy_self_attention_layer(nn.Module):
    ...
    Given 3 inputs of shape (batch_size, L, d_model), that will be used
    ...
    def forward(self,
        ...
        """Propagate forward the input through the MHB (Truncated Cauchy self-attention layer).
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Apply the Truncated Cauchy self-attention layer, add residual and normalize.
