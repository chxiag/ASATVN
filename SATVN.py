...
class SATVN(nn.Module):
    ...
    def __init__(self, input_dim, d_model, hidden_dim, h, M, output_dim, in_seq_length, out_seq_length, device):
        ...
        self.hidden_layer2 = nn.ModuleList(
            [Truncated_Cauchy_self_attention_block(input_dim, d_model, hidden_dim, output_dim, d_model, d_model, h, M, attention_size=12, dropout=0, chunk_mode=None,
    ...
    def forward(self, input, target, is_training=False):
        ...
        for i in range(self.out_seq_length):
            ...
            hidden = self.hidden_layer2[i](hidden)   # Propagate through Truncated Cauchy self-attention block
            ...
            output = self.generator[i](hidden)
            outputs[i,:,:] = output
